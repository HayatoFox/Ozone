...
---
*Document vivant — revue effectuée pour S1 → S41 ; à reconduire à chaque couche. À compléter par : groupement/horodatage des messages, vues MP/paramètres, câblage UI des autres capacités (rôles, modération…), stockage chiffré des jetons + `zeroize` (UI/registre), plafond de sessions/utilisateur + rate-limit des opcodes (R9), renégociation WS (mesh N-à-N) + E2EE DAVE/MLS (média) et leur audit, applications/bots/OAuth, rate-limiting REST (R1/R6), URLs signées pour pièces jointes, fuzzing du parseur gateway, tests de charge, et consommation transactionnelle des invitations (R5).*
