...
## 111. S111 — Lot « bugs & fonctionnalités » : uploads 1 Go annulables, vocal tier TeamSpeak, temps réel, **MP chiffrés de bout en bout** — **vérif adversariale**

Lot de correctifs/fonctionnalités issu d'un `/goal` utilisateur. Surface de sécurité **nouvelle** concentrée sur (a) le chiffrement E2EE des MP et (b) les uploads en flux jusqu'à 1 Go. Diagnostic des causes racines par fan-out multi-agents (5 explorateurs), puis implémentation + **test d'intégration adversarial dédié** (`tests/e2ee.rs`).

**Chiffrement de bout en bout des MP (migration 0029, `routes_users.rs`, `routes_messages.rs`, `lib/e2ee.ts`, `store.ts`)**
- **Modèle** : paire ECDH **P-256** par utilisateur. La clé **privée ne quitte jamais le poste** (localStorage, JWK) ; seule la clé **publique** (SPKI base64) est publiée (`PUT /users/@me/keys`) et lue (`GET /users/:id/keys`). Secret partagé dérivé par ECDH ⇒ clé **AES-GCM 256**. Le serveur ne stocke qu'un blob opaque `iv|ciphertext` dans `messages.cipher` ; **l'admin d'instance (accès BDD/SSH) ne peut pas lire les MP** — objectif utilisateur atteint.
- **(Critique — trouvé par le test, corrigé) Fuite de texte clair si `content` + `cipher` envoyés ensemble** : `create_message` stockait le `content` clair fourni à côté du `cipher`. Un client bogué/malveillant aurait persisté le clair en base. Corrigé : **dès qu'un `cipher` est présent, le serveur force `content = ""`** avant stockage, indexation FTS, auto-mod et traitement des mentions. Même invariant appliqué à l'édition (`edit_message` : `UPDATE … SET content = '', cipher = ?`). Prouvé par `dm_cipher_roundtrip` (le clair « ne doit pas fuir » n'est jamais restitué).
- **Périmètre verrouillé** : le chiffrement n'est accepté **qu'en MP** (gid == 0) — un `cipher` posté dans un salon de guilde renvoie **400** (`cipher_rejected_in_guild_channel`). Blob borné à **64 Ko**, clé publique bornée à **1024 octets** ; clé vide/surdimensionnée → **400**. Lecture des clés **authentifiée** (401 sans jeton).
- **Intégrité** : AES-GCM authentifie le chiffré → toute altération du blob est **rejetée** au déchiffrement (vérifié live dans le runtime navigateur : flip d'un octet ⇒ exception). Le serveur ne peut pas forger un MP lisible (il n'a pas la clé privée d'un participant).
- **Robustesse client** : pas de mise en cache des clés **absentes** (`null`/erreur) → pas de repli silencieux et permanent en clair pour la session ; repli en clair **uniquement** si le pair n'a pas (encore) publié de clé (interopérabilité). Échec de chiffrement ⇒ le message **n'est pas** envoyé en clair (on abandonne, on n'expose pas). Message indéchiffrable (clé locale différente, p. ex. autre appareil) ⇒ rendu « cadenas » explicite, jamais le blob brut. Réponses (`referenced_message`) chiffrées également déchiffrées.
- **Limites assumées (documentées)** : (i) **pas de sauvegarde de clé** → multi-appareil/effacement du stockage rend les anciens MP illisibles (prix de l'inaccessibilité serveur) ; (ii) les **pièces jointes** des MP ne sont pas (encore) chiffrées — seul le **texte** l'est ; (iii) pas de vérification d'empreinte de clé (TOFU implicite) → un admin pourrait théoriquement substituer une clé publique (MITM) : atténuation future = affichage/vérif d'empreinte.

**Uploads en flux jusqu'à 1 Go, annulables (`routes_attachments.rs`, `lib.rs`, `api.ts`, `ChatView.tsx`)**
- **Anti-DoS disque** : `upload_attachment` **streame** `field.chunk()` → fichier avec **contrôle de taille courant** ; dépassement de `MAX_ATTACHMENT_SIZE` (1 Gio) ⇒ arrêt + nettoyage. `DefaultBodyLimit` aligné (1 Gio + marge). Le téléchargement (`serve_attachment`) streame en chunks 64 Kio (`Body::from_stream`) → pas de chargement intégral en mémoire.
- **Annulation** : `AbortController` côté client (upload « staged » : on prépare le message, l'envoi part une fois l'upload fini) ⇒ l'annulation **coupe la requête** ; le rafraîchissement de jeton **avant** l'upload évite l'expiration en cours de transfert d'1 Go.
- **Réserve** : le rate-limit dédié aux uploads volumineux et les **quotas par utilisateur/instance** (remplissage disque par spam de gros fichiers sous la limite unitaire) restent à ajouter — noté en dette.

**Temps réel & permissions (`store.ts`, `routes_roles.rs`)** : application **en direct** des permissions (`CHANNEL_PERMISSIONS_UPDATE` en portée `Guild`, refetch débouncé des salons) — un salon caché par overwrite **disparaît sans reboot** ; aucune divulgation (le serveur retranche, le client ne fait que rafraîchir une vue déjà autorisée). Ordre des messages **déterministe** (tri snowflake, indépendant de l'ordre serveur). Badges/pile MP non-lus : purement présentationnel.

**Vocal (`ozone-sfu`, `lib/voice.ts`)** : keepalive WS (ping 20 s) anti-mort à ~10 s ; routage **RTCP PLI/FIR** pour la récupération de keyframe (anti-freeze) ; tuning Opus/jitter. **Sans impact d'authz** : le plan média reste authentifié par jeton vocal HS256 (secret partagé API↔SFU), nature des pistes en liste blanche (§85). Conforme à la consigne « on se fout de chiffrer le vocal si ça nuit aux perfs » : le SRTP du transport demeure, pas d'E2EE média ajoutée.

**Vérification** : `tests/e2ee.rs` **4/4** (publication/lecture de clé + bornes + auth ; aller-retour chiffré sans fuite de clair ; refus hors-MP ; édition chiffrée) ; **suite `cargo test -p ozone-api` complète au vert** ; `cargo check --workspace`, `tsc`, **42 tests client** verts. Live : round-trip ECDH+AES-GCM (accents/emoji OK, **altération rejetée**) validé dans le navigateur ; **`PUT /users/@me/keys → 200`** observé au boot du client (clé publiée) ; endpoints `keys` présents et **auth-gated** (401), route inexistante = 404 ; API redémarrée sur migration 0029 ; console sans erreur.

**Bilan** : E2EE des MP livré et **inaccessible à l'admin d'instance** (objectif utilisateur) ; **1 fuite de texte clair critique fermée** (capturée par le test d'intégration avant tout déploiement) ; périmètre E2EE verrouillé aux MP, blobs/clé bornés, intégrité GCM. Uploads 1 Go en flux sans charge mémoire/disque non bornée côté traitement. **Aucune faille exploitable** introduite ; dettes notées (quota disque uploads, sauvegarde/empreinte de clé, E2EE des PJ).

## 112. S112 — Persistance multi-appareils de l'E2EE (escrow chiffré + auth zero-knowledge) — **vérif adversariale**

Avant ce lot, la clé privée DM ne vivait qu'en `localStorage` ⇒ un autre appareil (ou un stockage vidé) perdait l'accès aux anciens MP. Option A retenue : **dépôt de la clé privée chiffrée côté serveur, sous une clé dérivée du mot de passe que le serveur ne voit jamais** (migration 0030, `routes_auth.rs`, `routes_users.rs`, `lib/e2ee.ts`).

**Schéma cryptographique (client)** : `masterKey = PBKDF2-HMAC-SHA256(mot de passe, sel, 600 000)`, puis **HKDF** → `authSecret` (envoyé ; le serveur stocke `Argon2(authSecret)`) **et** `KEK` (ne quitte JAMAIS le poste). La clé privée DM est emballée `AES-GCM(KEK, JWK)` et déposée (`dm_priv_wrapped`). N'importe quel appareil : `prelogin` (sel) → dérivation → `login(authSecret)` → `GET /users/@me/encryption` → déballage avec la KEK → **tous les MP, passés et futurs**.
- **Modèle de menace (admin BDD/SSH)** : voit `authSecret` (un hash → ne remonte pas au mot de passe → ni à la KEK), la clé privée **emballée** (blob AES-GCM opaque), la clé publique, le sel. **Ne peut pas déchiffrer.** Garantie préservée. ✓
- **Anti-énumération du `prelogin`** : compte inconnu ou v1 → **sel factice déterministe** `SHA-256(secret JWT | login)` (64 hex), indistinguable d'un vrai sel et **non précalculable** (lié au secret serveur) ; `pw_scheme = 2` renvoyé ⇒ le client poursuit et reçoit un 401 normal. Vérifié : `prelogin_returns_salt_and_hides_unknown`.
- **Migration v1→v2 (transparente)** : un compte legacy (`Argon2(mdp brut)`) bascule au prochain login : login au mot de passe brut **une dernière fois**, puis `POST /users/@me/encryption/upgrade` re-prouve le mot de passe et remplace le hash par `Argon2(authSecret)`, `pw_scheme = 2`, dépose l'escrow. Le mot de passe brut ne transite qu'à cette occasion unique. `provisionKeys` **réutilise la clé locale existante** lors de la migration ⇒ **les MP existants restent déchiffrables** et deviennent portables. Vérifié : `legacy_upgrade_to_v2` (login authSecret OK, mot de passe brut ensuite **refusé**, re-migration interdite).
- **Changement de mot de passe (v2)** : le client ré-emballe la clé privée avec la **nouvelle** KEK (sel inchangé) et envoie `priv_wrapped` ⇒ l'escrow reste déchiffrable. Si la clé locale manque, on déballe d'abord l'escrow avec l'**ancienne** KEK puis on ré-emballe → **pas de verrouillage**. Vérifié : `change_password_rewraps_escrow`.
- **Déconnexion** : `e2eeForget()` purge clé/sel/caches locaux ⇒ un autre utilisateur sur le même poste ne peut pas **hériter** de la clé du précédent (sinon `provisionKeys` l'aurait réutilisée à tort lors de SA migration).
- **Boot (jeton stocké, sans mot de passe)** : `ensureKeypair` ne fait plus que **charger** la clé en cache — **ne génère ni ne publie plus**. Ceci corrige aussi le **bug d'origine de perte de clé** (chaque client neuf générait une nouvelle paire et écrasait la clé publique).
- **Bornes** : escrow ≤ 8 Ko, clé publique ≤ 1 Ko, sel ≤ 128, `authSecret` ≤ 1 Ko. **Politique de robustesse (R3)** : appliquée **côté client** en v2 (le serveur ne voit que l'authSecret) — affaiblissement assumé et documenté.

**Limites (inchangées ou nouvelles)** : (i) **web** = JS servi par le serveur → un serveur *activement* malveillant pourrait exfiltrer le mot de passe/KEK ; le **.exe Tauri** (code empaqueté/signé) lève l'essentiel de ce risque — protège contre l'admin honnête-mais-curieux / l'accès BDD a posteriori. (ii) **Mot de passe oublié = MP irrécupérables** jusqu'à la **Phase 2 (code de récupération)**. (iii) Pas d'**empreinte de clé** (TOFU) ⇒ un admin pourrait substituer une clé publique (MITM) ; atténuation future = affichage d'empreinte. (iv) Les **pièces jointes** des MP ne sont toujours pas chiffrées (texte seul).

**Vérification** : `cargo test -p ozone-api` **complète au vert** dont `tests/e2ee.rs` **8/8** (escrow v2, migration v1→v2, ré-emballage au changement de mdp, prelogin + anti-énumération). Client : `e2ee.test.ts` (déterminisme/dépendance de la dérivation, **récupération multi-appareils** wrap→unwrap, rejet d'altération), `tsc`, **vitest 46/46**. Live : migration 0030 appliquée, endpoints up (`prelogin` 200, escrow **401-gated**), **session existante survécue au redémarrage**, console sans erreur.
...
*Document vivant — revue effectuée pour S1 → S86 ; à reconduire à chaque couche. **Transport média vocal livré et audité (S84-S86)** : WebRTC ↔ `ozone-sfu`, plan média authentifié (jeton vocal HS256), attribution des flux par membre sûre par construction (identité serveur, nature en liste blanche), caméra-par-personne + partage d'écran (Go Live) + détection de parole, qualité/latence Opus réglées ; `ring` cantonné au nœud média. **Résolus depuis** : R1/R6 (rate-limiting REST, §109), R3/R5 (politique de mot de passe + invitations atomiques, §108), R9 (plafond de sessions, §108 — le rate-limit des **opcodes Gateway** reste à faire), **E2EE des MP (texte) livré (§111)** — inaccessible à l'admin d'instance ; restent l'E2EE des **pièces jointes** de MP, la **sauvegarde/empreinte de clé** (anti-MITM), et un **quota disque** sur les uploads volumineux. À compléter par : R11 (unfurling OG serveur), **E2EE média** (DAVE/MLS) + TURN/production et leur audit, durcissement du stockage des jetons (R10 : stockage sécurisé OS via Tauri + CSP stricte), rate-limit des opcodes Gateway (IDENTIFY/RESUME), R2 (vérification d'e-mail anti-énumération), applications/bots/OAuth, fuzzing du parseur gateway, et tests de charge.*
